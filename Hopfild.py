...
#
    ...
    def __hopfield(self,train_files):
        ...
        #     r = self.__readImg2array(train_files[i], size)
        #     x = self.__mat2vec(r)
        #     tmp_w = self.__create_W(x)
        #     w =w+tmp_w

#название имен одиннаковое проверить
#
        # x_er = self.__readImg2array(test_files, size)
        # y = self.__mat2vec(x_er)
        # z = self.__vectomat(y)
        #
        # q = np.dot(w, y)
        ...
        #Эта функция получвет на вход массив
    # def hopfield2(self, train_files, test_files, size):
    #         num_files = 0
    #         for path in train_files:
    #
    #             x = self.__readImg2array(path, size)
    #             x_vec = self.__mat2vec(x)
    #
    #             if num_files == 0:
    #                 w = self.__create_W(x_vec)
    #                 num_files = 1
    #             else:
    #                 tmp_w = self.__create_W(x_vec)
    #                 w = np.sum(w, tmp_w)
    #                 num_files += 1
    #
    #         # r = self.__readImg2array(train_files, size)
    #         # x = self.__mat2vec(r)
    #         # w = self.__create_W(x)
    #
    #         rr = self.__readImg2array(test_files, size)
    #         y = self.__mat2vec(rr)
    #         z = self.__vectomat(y)
    #
    #         q = np.dot(w, y)
